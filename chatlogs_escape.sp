...
void addToSQL(const char[] command, int client, const char[] text, bool adminText = false)
{
	...
	char sQuery[1024], sQueryEscaped[1024];
	...
	Format(sQuery, sizeof(sQuery), "INSERT INTO `chat_logs` (`time`, `tick`, `ip`, `port`, `map`, `command`, `name`, `communityid`, `alive`, `team`, `text`) VALUES (UNIX_TIMESTAMP(), %d, \"%s\", %d, \"%s\", \"%s\", \"%s\", \"%s\", %d, \"%s\", \"%s\")", iTick, sIP, iPort, sMap, command, sName, sAuth, bAlive, sTeam, text);
	g_dDB.Escape(sQuery, sQueryEscaped, sizeof(sQueryEscaped));
